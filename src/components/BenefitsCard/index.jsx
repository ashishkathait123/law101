...
const BenefitsCard = () => {
  return (
    <div
      id="id-benefits-card"
      className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-6 px-4 sm:px-6 md:px-12 py-8"
    >
      {BenefitsCardsdata.map((item, i) => (
        <BenefitCard key={i} cardData={item} />
      ))}
    </div>
  );
};
...
const BenefitCard = ({ cardData }) => {
  ...
  return (
    <div className="p-4 flex flex-col justify-between gap-4 rounded-xl bg-white text-gray-700 shadow-md">
      <div className="flex items-center justify-start">{imageSvg}</div>
      <h5 className="text-lg font-semibold text-blue-900">{title}</h5>
      <p className="text-sm text-gray-600">{con}</p>
      <div>
        <a href="#" className="text-sm text-blue-600 hover:text-pink-500 font-medium flex items-center gap-2 mt-2">
          Learn More
          <svg
...
const BenefitsCardsdata = [
  {
    imageSvg: (
      <svg xmlns="http://www.w3.org/2000/svg" height="45" fill="#335e9e" viewBox="0 -960 960 960" width="45">
  ...
  {
    imageSvg: (
      <svg xmlns="http://www.w3.org/2000/svg" fill="#335e9e" height="45" viewBox="0 -960 960 960" width="45">
